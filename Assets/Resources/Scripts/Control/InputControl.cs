...
public class InputControl : Core.MonoSingleton<InputControl> {
	...
	void Update () {
		...
		if (Input.GetMouseButtonDown (0)) {
			...
			//Board.Instance.MoveBottomRightFrom(currentPressedPosition);
		}
		if (Input.GetMouseButtonUp (0)) {
			...
			{
				Board.Instance.MoveFrom(currentPressedPosition,BoardDirection.Right);
			}
			else if(angle< 60f + thred && angle> 60f - thred)
			{
				Board.Instance.MoveFrom(currentPressedPosition,BoardDirection.TopRight);
			}
			else if(angle< 120f + thred && angle> 120f - thred)
			{
				Board.Instance.MoveFrom(currentPressedPosition,BoardDirection.TopLeft);
			}
			else if(angle< 180f + thred && angle> 180f - thred)
			{
				Board.Instance.MoveFrom(currentPressedPosition,BoardDirection.Left);
			}
			else if(angle< 240f + thred && angle> 240f - thred)
			{
				Board.Instance.MoveFrom(currentPressedPosition,BoardDirection.BottomLeft);
			}
			else if(angle< 300f + thred && angle> 300f - thred)
			{
				Board.Instance.MoveFrom(currentPressedPosition,BoardDirection.BottomRight);
			}
